...
while ($true) {
    ...
    $DtPastTime= (Get-Content Dt.ctr)
    If ($DtPastTime -eq $null -or $DtPastTime -eq "" ) {$DtPastTime=0}
    $ElapsedDtTime = [int]($DtPastTime) + $LastIntervalTime.minutes + ($LastIntervalTime.hours *60)

    
    $Dt= [int](get-config-variable "DONATE")
    $DtTime=if ($Dt -gt 12) {[int]$Dt} else {12}
    #Activate or deactivate dt
    if ($ElapsedDtTime -gt 1440 -and $DtTime -gt 0) { # dt interval

                $DtInterval = $true
                $NextInterval=$DtTime *60
                ...
                0 | Set-Content  -Path Dt.ctr

            }
            else { #NOT dt interval
                    $DtInterval = $false
                    ...
                    $ElapsedDtTime | Set-Content  -Path Dt.ctr
